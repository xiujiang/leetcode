 ...
 * 你可以无限次地完成交易，但是你每笔交易都需要付手续费。如果你已经购买了一个股票，在卖出它之前你就不能 再继续购买股票了。
 * 返回获得利润的最大值。
 *
 ...
 * 0 <= fee < 50000.
 */
public class SevenHandredFourteen {
    public static void main(String[] args) {
        int[] x = new int[]{1, 3, 2, 8, 4, 9};
        System.out.println(maxProfit(x,2));
    }
    static int maxProfit(int[] prices, int fee) {
        ...
        for(int i = 0; i < prices.length; i++){
            if(minPrice > prices[i]){
                minPrice = prices[i];
            }else if((prices[i] - minPrice-2) > 0){
                res += prices[i] - minPrice-2;
                minPrice = prices[i];
            }
        }
        return res;
